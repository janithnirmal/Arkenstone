...
const ProductForm: React.FC<Props> = ({ productToEdit, onSuccess }) => {
    ...
    useEffect(() => {
        // Pre-fill form if we are editing a product
        if (productToEdit) {
            reset({
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex items-start">
                <div className="flex h-5 items-center">
                    <input id="is_active" type="checkbox" {...register('is_active')} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                </div>
